...
const PaymentMethod = ({
  ...
  function calculateHash(merchant_id, order_id, amount, currency, merchant_secret) {
    // Formatting the amount with 2 decimal places
    amount = number_format(amount);

    // Creating the hash string
    let hashString = merchant_id + order_id + amount + currency + md5(merchant_secret);
  ...
  // Example usage:
  let merchant_id = '1228377';
  let order_id = uuid();
  let currency = 'LKR'; // Replace with your currency
  let merchant_secret = 'MTYyNDk5ODEzMjI2NjUyNzcyNDQxODYyNjE0NDE4Mzg4OTM0Mjk5';

  let hash = calculateHash(merchant_id, order_id, amount, currency, merchant_secret);
